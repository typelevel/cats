...
class ConstSuite extends CatsSuite {
  ...
  checkAll("Const[String, Int]", DivisibleTests[Const[String, ?]].divisible[Int, Int, Int])
  checkAll("Divisible[Const[String, ?]]", SerializableTests.serializable(Divisible[Const[String, ?]]))
