...
object GenTupleCoreInstances extends Template {
  ...
  def content(tv: TemplateVals): String = {
    ...
    |private[instances] trait NTupleInstances extends NTupleInstances1 {
      ...
      -
      -      override def map[A, B](fa: ${`A0, A(N - 1) & `("A")})(f: A => B): ${`A0, A(N - 1) & `("B")} = ${`fa._1..(n - 1) & `(
      s"f(fa._$arity)"
    )}
      -
      ...
      -
      -      override def reduceLeftToOption[A, B](fa: ${`A0, A(N - 1) & `("A")})(f: A => B)(g: (B, A) => B): Option[B] =
      -        Some(f(fa._$arity))
      -
      -      override def reduceRight[A](fa: ${`A0, A(N - 1) & `("A")})(f: (A, Eval[A]) => Eval[A]): Eval[A] =
      -        Now(fa._$arity)
      -
      -      def reduceRightTo[A, B](fa: ${`A0, A(N - 1) & `("A")})(f: A => B)(g: (A, Eval[B]) => Eval[B]): Eval[B] =
      -        Now(f(fa._$arity))
      -
      -      override def reduceRightToOption[A, B](fa: ${`A0, A(N - 1) & `("A")})(f: A => B)(g: (A, Eval[B]) => Eval[B]): Eval[Option[B]] =
      -        Now(Some(f(fa._$arity)))
      -
      -      override def reduceMap[A, B](fa: ${`A0, A(N - 1) & `("A")})(f: A => B)(implicit B: Semigroup[B]): B =
      -        f(fa._$arity)
      -
      -      override def size[A](fa: ${`A0, A(N - 1) & `("A")}): Long = 1L
      -
      -      override def get[A](fa: ${`A0, A(N - 1) & `("A")})(idx: Long): Option[A] =
      -        if (idx == 0L) Some(fa._$arity) else None
      -
    ...
    |}
      ...
      -  override def ap[A, B](ff: ${`A0, A(N - 1) & `("A => B")})(fa: ${`A0, A(N - 1) & `("A")}): ${`A0, A(N - 1) & `(
      "B"
    )} =
      ...
      -  }
      -
      -  override def productR[A, B](a: ${`A0, A(N - 1) & `("A")})(b: ${`A0, A(N - 1) & `("B")}): ${`A0, A(N - 1) & `(
      "B"
    )} =
      -    ${`combine A..(N - 1)`("a", "b", s"b._$arity")}
      -
      -  override def productL[A, B](a: ${`A0, A(N - 1) & `("A")})(b: ${`A0, A(N - 1) & `("B")}): ${`A0, A(N - 1) & `(
      "A"
    )} =
