...
class OptionSuite extends CatsSuite {
  ...
  checkAll("Option[Int] with Option", TraverseEmptyTests[Option].traverseEmpty[Int, Int, Int])
  checkAll("TraverseEmpty[Option]", SerializableTests.serializable(TraverseEmpty[Option]))
