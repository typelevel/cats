...
class FunctionKSuite extends CatsSuite {
  ...
  test("id is identity") {
    forAll((list: List[Int]) => FunctionK.id[List].apply(list) should ===(list))
  }
  ...
  test("lift simple unary") {
    ...
    forAll((a: Option[Int]) => fOptionToList(a) should ===(optionToList(a)))

    val fO2I: FunctionK[Option, Iterable] = FunctionK.lift(Option.option2Iterable _)
    forAll((a: Option[String]) => fO2I(a).toList should ===(Option.option2Iterable(a).toList))

    val fNelFromListUnsafe = FunctionK.lift(NonEmptyList.fromListUnsafe _)
    forAll((a: NonEmptyList[Int]) => fNelFromListUnsafe(a.toList) should ===(NonEmptyList.fromListUnsafe(a.toList)))
  }
  ...
  test("lift compound unary") {
    ...
    forAll((a: List[String]) => fNelFromList(a) should ===(NonEmptyList.fromList(a)))
  }
