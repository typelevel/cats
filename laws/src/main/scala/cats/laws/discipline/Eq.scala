...
import cats.data.{AndThen, EnvT, RepresentableStore}
import cats.Eq
...
object eq {
  ...
  implicit def catsLawsEqForEnvT[W[_], E, A](implicit eqFA: Eq[W[A]], eqS: Eq[E]): Eq[EnvT[W, E, A]] =
    Eq.instance((s1, s2) => eqFA.eqv(s1.wa, s2.wa) && eqS.eqv(s1.env, s2.env))
}
