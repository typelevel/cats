...
trait DecidableTests[F[_]] extends ContravariantMonoidalTests[F] {
  ...
  def decidable[A: Arbitrary, B: Arbitrary, C: Arbitrary](implicit
    ...
    iso2: DecidableTests.Isomorphisms[F]
  ): RuleSet =
    new RuleSet {
      val name = "decideable"
      ...
      val props = Seq(
        ...
        "decideable sum associativity" -> forAll((fa: F[A], fb: F[B], fc: F[C]) =>
          iso2.associativity(laws.decideableSumAssociativity[A, B, C](fa, fb, fc))
        ),
...
object DecidableTests {
  def apply[F[_]: Decidable](implicit ev: Isomorphisms[F]): DecidableTests[F] =
