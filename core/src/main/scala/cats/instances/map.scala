...
trait MapInstances extends cats.kernel.instances.MapInstances {
  ...
  implicit def catsStdInstancesForMap[K]: TraverseFilter[Map[K, ?]] with Traverse[Map[K, ?]] with FlatMap[Map[K, ?]] with RecursiveTailRecM[Map[K, ?]] =
    new TraverseFilter[Map[K, ?]] with Traverse[Map[K, ?]] with FlatMap[Map[K, ?]] with RecursiveTailRecM[Map[K, ?]] {
