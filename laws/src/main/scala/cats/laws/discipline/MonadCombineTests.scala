...
trait MonadCombineTests[F[_]] extends MonadFilterTests[F] with MonoidKTests[F] with AlternativeTests[F] {
  ...
  def monadCombine[A: Arbitrary, B: Arbitrary, C: Arbitrary](implicit
    ...
    new RuleSet {
      ...
      def bases = Nil
      def parents = Seq(monadFilter[A, B, C], monoidK[A], alternative[A,B,C])
      def props = Seq(
