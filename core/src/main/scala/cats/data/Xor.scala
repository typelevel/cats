...
private[data] sealed abstract class XorInstances extends XorInstances1 {
  ...
  implicit def catsDataInstancesForXor[A]: Traverse[A Xor ?] with Monad[A Xor ?] with MonadError[Xor[A, ?], A] =
    new Traverse[A Xor ?] with Monad[A Xor ?] with MonadError[Xor[A, ?], A] {
