...
object FunctionK {
  protected type α <: Object with Nothing
   ...
   * }}}
   *
   * Note: The abstract `α` parameter is there to compensate for
   * the lack of polymorphic function types in Scala 2.
   */
  def lift[F[_], G[_]](f: F[α] => G[α]): FunctionK[F, G] = new FunctionK[F, G] {
    def apply[A](fa: F[A]): G[A] = f(fa.asInstanceOf[F[α]]).asInstanceOf[G[A]]
  }
}
