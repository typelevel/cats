...
ns: cats.data.Streaming[Int] = Streaming(1, ...)

scala> val xs = ns.filter(_ < 50)
xs: cats.data.Streaming[Int] = Streaming(1, ...)

scala> val ys = ns.map(_ * 2 + 1)
ys: cats.data.Streaming[Int] = Streaming(3, ...)

scala> val zs = xs ++ ys
zs: cats.data.Streaming[Int] = Streaming(1, ...)
...
int(2)
int(3)
now: cats.data.Streaming[Int] = Streaming(1, ...)

scala> val later = int(1) %:: int(2) %:: int(3) %:: Streaming.empty[Int]
int(1)
later: cats.data.Streaming[Int] = Streaming(1, ...)
```
...
fives: cats.data.Streaming[Int] = <lazy>
...
createMmix: (seed: Long)cats.data.Streaming[Long]

scala> val longs: Streaming[Long] = createMmix(0x741f2d2eea7e5c70L)
longs: cats.data.Streaming[Long] = Streaming(8367456312203566192, ...)
...
scala> val ints: Streaming[Int] =
     ...
     |     Streaming(n0, n1)
     |   }
ints: cats.data.Streaming[Int] = Streaming(1948200238, ...)
```
...
str: (xs: cats.data.Streaming[Int])String

scala> val h = pi.take(1)
h: cats.data.Streaming[Int] = Streaming(3, ...)

scala> val t = pi.drop(1).take(40)
t: cats.data.Streaming[Int] = Streaming(...)
