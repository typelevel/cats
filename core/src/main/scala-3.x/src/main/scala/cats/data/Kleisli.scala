...
object Kleisli extends KleisliInstances
  ...
  def apply[F[_], A, B](run: A => F[B]): Kleisli[F, A, B] = run

  /* Extensions that need no implicits */
  extension[F[_], A, B](x: Kleisli[F, A, B]) {
    def apply(a: A): F[B] = x(a)
    ...
    def toReader: Reader[A, F[B]] = x

    /**
      * Modify the context `F` using transformation `f`.
      */
    def mapK[G[_]](f: F ~> G): Kleisli[G, A, B] = a => f(x(a))
  ...
  /* Extensions that need Functor on F */
  extension[F[_], A, B](x: Kleisli[F, A, B])(using F: Functor[F]) {
    ...
    def first[C]: Kleisli[F, (A, C), (B, C)] =
      { case (a, c) => F.fproduct(x(a))(_ => c) }

    def second[C]: Kleisli[F, (C, A), (C, B)] =
      { case (c, a) => F.map(x(a))(c -> _) }

    /**
      * Discard computed B and yield the input value.
      */
    def tap[AA <: A]: Kleisli[F, AA, AA] = a => F.as(x(a), a)

    /**
      * Yield computed B combined with input value.
      */
    def tapWith[C, AA <: A](f: (AA, B) => C): Kleisli[F, AA, C] = a => F.map(x(a))(b => f(a, b))

  }

  extension[F[_], A, B](x: Kleisli[F, A, B])(using F: Applicative[F]) {

    def traverse[G[_], AA <: A](f: G[AA])(using G: Traverse[G]): F[G[B]] =
      G.traverse(f)(x)

    def lower: Kleisli[F, A, F[B]] = a => F.pure(x(a))
  }

  /* Extensions that need Functor on F */
  extension[F[_], A, B](x: Kleisli[F, A, B])(using F: FlatMap[F]) {

    /**
      * Composes [[run]] with a function `B => F[C]` not lifted into Kleisli.
      */
    def andThen[C](k: Kleisli[F, B, C]): Kleisli[F, A, C] =
      shift(a => F.flatMap(x(a))(k.run(_)))

    def compose[Z, AA <: A](k: Kleisli[F, Z, AA]): Kleisli[F, Z, B] =
      shift((z: Z) => F.flatMap(k.run(z))(x) )

    def tapWithF[C, AA <: A](f: (AA, B) => F[C]): Kleisli[F, AA, C] =
      a => F.flatMap(x(a))(b => f(a, b))

    def flatMap[C, AA <: A](f: B => Kleisli[F, AA, C]): Kleisli[F, AA, C] =
      shift(a => F.flatMap[B, C](x(a))((b: B) => f(b).run(a)))

    def flatMapF[C](f: B => F[C]): Kleisli[F, A, C] =
      shift(a => F.flatMap(x(a))(f))

  }

  extension[F[_], A, C, D](x: Kleisli[F, A, C => D])(using F: Apply[F]) {
    def ap[AA <: A](f: Kleisli[F, AA, C]): Kleisli[F, AA, D] =
      a => F.ap(x(a))(f(a))

  }

  extension[F[_], A, B](x: Kleisli[F, A, B])(using F: Apply[F]) {
    def pa[C, AA <: A](f: Kleisli[F, AA, B => C]): Kleisli[F, AA, C] =
      a => F.ap(f.run(a))(x(a))
...
sealed private[data] trait KleisliFunctionsBinCompat {
  ...
  def liftFunctionK[F[_], G[_], A](f: F ~> G): Kleisli[F, A, *] ~> Kleisli[G, A, *] =
    new (Kleisli[F, A, *] ~> Kleisli[G, A, *]) {
      def apply[B](k: Kleisli[F, A, B]): Kleisli[G, A, B] = k.mapK(f)
    }
}
