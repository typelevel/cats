...
@deprecated(message = "Deprecated in favor of cats.data.EitherT - import cats.implicits._ or cats.syntax.either._ to get missing syntax for Either", since = "0.8.0")
class XorTTests extends CatsSuite {
  ...
  test("toValidated") {
    forAll { (xort: XorT[List, String, Int]) =>
      xort.toValidated.map(_.toEither) should === (xort.value.map(_.toEither))
    }
  }

  test("toValidatedNel") {
    forAll { (xort: XorT[List, String, Int]) =>
      xort.toValidatedNel.map(_.toEither.leftMap(_.head)) should === (xort.value.map(_.toEither))
    }
  }
